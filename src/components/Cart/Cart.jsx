...
function Cart() {
  ...
  function handleBuyNow() {
    ...
    navigate("/Buynow", {state: {items: cartItems,total: totalPrice,},
    });
  }
  ...
  return (
    <div className="cart-page">
      ...
      <h2 className="loading-title">Loading Cart...</h2>
    </div>
      ) : cartItems.length === 0 ? (
      ...
      <tr key={item.id}>
        <td className="product-cell">
          <img
            ...
            src={Array.isArray(item.image)? item.image[0]: item.image} alt={item.name}/>
          <span>{item.name}</span>
        </td>
...
<td>
  <div className="qty-box">
    ...
    <div className="qty-icons">
      <i className="bi bi-chevron-up" onClick={() => increaseQty(item)}></i>

      <i className="bi bi-chevron-down" onClick={() => decreaseQty(item)}></i>
    </div>
  </div>
</td>
